...
function createMap(target, param, object) {
    ...
    if (basemap === 'google') {
        ...
        if (typeof google === 'object' && typeof google.maps === 'object') {
            ...
            script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyD5LLegdsHGdHj-9FlRS-W8r8czoUR-0_4&callback=initMap';
            script.async = true;
    ...
    if (basemap === 'google') {
        ...
        if (currentMap.type === 'ol') {
            if (basemap !== currentMap.basemap) {
                ...
                layer = returnBasemapLayer(param.cartography.basemaps[basemap].source);
...
function createBasemapLayers(param) {
    try {
        ...
        for (let i = 0; i < basemaps.length; i++) {
            ...
            let layer = returnBasemapLayer(source, preload);
            if (layer) {
                updated[item.name] = {
                    layer: layer,
                    source: source,
...
function returnBasemapLayer(source, preload) {
    try {
        if (source.type === 'OSM') {
...
function createMenuMap(div, target, object, param) {
    ...
    let layer = returnBasemapLayer(source);
