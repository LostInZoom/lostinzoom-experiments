        ...
        function constructDiv(param, page, content, element) {
            ...
            //Handling lists
                ...
                function createQuestion(object, cascading, hidden) {
                    ...
                    if ('svg' in object.question) {
                        ...
                        let svg = makeElement('form-svg form-svg-question', `<img src='../static/fogdetector/img/${name}.svg' />`);
                    ...
                    for (let j = 0; j < object.items.length; ++j) {
                        ...
                        if (e.type === 'button' || e.type === 'svg') {
                            ...
                            for (let k = 0; k < content.length; ++k) {
                                ...
                                let button;
                                if (svg) {
                                    button = makeElement('form-svg-answer form-svg form-item form-item' + nbQuestion + status, `<img src='../static/fogdetector/img/${content[k]}.svg' />`, id);
            ...
            //Handling image type
                ...
                for (let img = 0; img < object.content.length; ++img) {
                    ...
                    let imagecontent;
                    if ('link' in image) {
                        imagecontent = `<a href='${image.link}' target='_blank'><img src='../static/fogdetector/img/${file}'/></a>`
                    } else {
                        imagecontent = `<img src='../static/fogdetector/img/${file}'/>`
                    }
            ...
            //Handling results type
                ...
                if (param.results) {
                    let resultsContainer = makeElement('results-container');
                    Object.keys(param.results.user).forEach(function(key) {
                        let result = makeElement('results-item')
                        let Uresult = makeElement('results-item-element results-item-user');
                        let Uval = makeElement('results-item-value', param.results.user[key] + ' ' + param.global.texts.results[key].unit[language]);
                        let Ulabel = makeElement('results-item-label', param.global.texts.results[key].label[language]);
                        let Lresult = makeElement('results-item-element results-item-all');
                        let Lval = makeElement('results-item-value', param.results.all[key] + ' ' + param.global.texts.results[key].unit[language]);
                        let Llabel = makeElement('results-item-label', param.global.texts.results[key].label[language]);
                        let overall = makeElement('results-item-label results-item-label-overall', param.global.texts.results.overall[language]);
                        Uresult.append(Uval, Ulabel);
                        Lresult.append(overall, Lval, Llabel);
                        result.append(Uresult, Lresult);
                        resultsContainer.appendChild(result);
                    });
                    pageContent.appendChild(resultsContainer);
                }
            }
